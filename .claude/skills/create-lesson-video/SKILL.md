...
- **URL mode**: use `WebFetch` with a prompt asking for `title`, `content` (~500–1500 words), `ogImage`, `domain` as JSON. If blocked (paywall/JS/4xx) → ask user to paste material into a `.txt` file. Stop.
- **File mode**: use `Read`. Title = first non-empty line (or `#` heading for .md), content = rest. ogImage = `null`, domain = `"local"`.
- **Topic mode**: write the outline from the argument; title = the topic. ogImage = `null`, domain = `"lesson"`.

### Step 3: Slug + output dir
Same as news skill: slug = lowercase ASCII (strip diacritics, đ→d, non-alnum → `-`, ≤40 chars); outputDir = `output/<slug>-<YYYYMMDD-HHmm>/`. Create it with Bash: `mkdir -p <outputDir>`.
...
Use the Write tool → `<outputDir>/script.json`.

### Step 7: Run the pipeline
Use Bash, foreground, stream output:
```bash
...
Short Vietnamese caption + exactly 4 hashtags → `<outputDir>/caption.txt` via the Write tool.
