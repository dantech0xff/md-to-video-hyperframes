...
- **Sửa kịch bản bằng form (2.3):**
  ...
  - Storyboard ghi lại nó được dựng từ nội dung kịch bản nào (`storyboard.inputs.json`: dấu vân tay của kịch bản và thời điểm ảnh đổi lần cuối). Kịch bản bị sửa trong lúc dựng (agent sửa ngay sau khi người dùng lưu) thì storyboard vẫn bị coi là cũ, dù nó được ghi sau kịch bản. Video render cũng ghi như vậy (`video.inputs.json`), và danh sách dự án đọc bản ghi đó để biết video có cũ không. Thời điểm của mỗi ảnh là lúc engine đọc ảnh đó, nên ảnh bị thay trong lúc đọc lời thoại (trước khi được chép) không làm output vừa dựng bị coi là cũ. Storyboard và video của engine cũ, chưa có bản ghi, vẫn so theo thời gian như trước.
